function [A_0,timer,tno] = compute_periodic_perturb(obj, order)
...
%         A_0 = repmat(struct('coeffs',[]),1,order);
        A_0 = repmat(struct('coeffs',[]),1,order);
...
%         [W_0(1),R_0(1),multi_input] = coeffs_setup(obj,order,DStype);
        [A_0(1), multi_input] = coeffs_setup_periodic_perturb(obj,order);
        % *Outer resonance case* : issue warning
%         if obj.resonance.outer.occurs
%             prompt = 'Due to (near) outer resonance, the exisitence of the manifold is questionable and the underlying computation may suffer.';
%             disp(prompt)
...
%         tno =multi_input.time_no;
        for j = 2:order
            %recursively approximate at j-th order
            startOrderj = tic;
%             [W_0(j).coeffs,R_0(j).coeffs,multi_input] = cohomological_solution(obj,j,W_0,R_0,multi_input,DStype);
%             [A_0(j).coeffs,multi_input] = cohomological_solution_anchor(obj,j,A_0,multi_input);
               [A_0(j).coeffs,multi_input,timer] = cohomological_solution_periodic_perturb(obj,j,A_0,multi_input);
            obj.solInfo.timeEstimate(j) = toc(startOrderj);
            disp(['Anchor computation time at order ' num2str(j) ' = ' datestr(datenum(0,0,0,0,0,obj.solInfo.timeEstimate(j)),'HH:MM:SS')])
            fprintf('Estimated memory usage at order % 2i = %05.2E MB\n', j, obj.solInfo.memoryEstimate(j))            
        end
%         [A_0] = coeffs_lex2revlex(A_0,'TaylorCoeff');
%         switch DStype
%             case 'real'
...
%         end
        ...
        [A_0] = coeffs_output_anchor(A_0,order);
