function [W_0i, R_0i,multi_input,timer] = cohomological_solution_aperiodic_whisker(obj, i,W_0, R_0,multi_input)
...
switch obj.Options.notation
    ...
    case 'multiindex'
        ...
        B   = obj.System.B; % B matrix
        N   = obj.dimSystem; % Full system dimensionality in first-order form
        l   = numel(obj.E.spectrum) + 1; % dim(M): M is the master modal subspace
        F   = obj.System.F; % Full system Nonlinearity coefficients at different orders
        Mi = obj.System.Minv;
...
%         end
        ...
        % Mixed Terms
        WR = zeros(N,z_k,1,Ntime);
        
        for m = 2:k-1
            WR = WR + coeffs_mixed_terms_aperiodic(k,m,W_0,R_0,multi_input,'aut'); % Dependent on ordering, chooses conj, or revlex computation
        end
        ...
        H_k  = coeffs_composition_aperiodic(W_0,H,multi_input); % Dependent on ordering, chooses conj, or revlex computation
        H{k} = H_k;
        multi_input.H = H;
        
        %% Nonlinearity terms
        % Now the nonlinearity contribution for the equation at order k is computed
        Fn = zeros(N,z_k,Ntime);
        for n = 2:min(k,multi_input.nl_order) % k+1 since this term includes a derivative
            if ~isempty(F(n)) && ~isempty(F(n).coeffs) && ~isempty(F(n).ind)
                Fn  = Fn + pagemtimes(full(F(n).coeffs), ...
                     compute_pi_aperiodic(F(n).ind.',K,multi_input)); % Dependent on ordering, chooses conj, or revlex computation
            end
        end    
        Lambda_K         = sum(K(2:end,:).*Lambda_M_vector);
        Epsilon_indices = find(K(1,:));
        R_k = zeros(l-1,z_k,1,max(size(multi_input.ttime)));
        W_k = zeros(2*(obj.System.n),z_k,1,max(size(multi_input.ttime)));

%         for f = Epsilon_indices
%         JOP = squeeze(Fn(:,f,:));
% 
...
%                     residual = @(q,qd,qdd,t) residual_linear_inhomo( q, qd, qdd, t, M,Ks,C, F_ext);
% 
%                     % Time Integration
%                     tic 
...
%                   Cm = [-Fext;sparse(obj.System.n,max(size(Fext)))]-B*squeeze(WR(:,f,1,:));
%                   Ft = Cm;
% %                   Ft = [-Fext;sparse(obj.System.n,max(size(Fext)))];
%                   phi = W'*Ft;% (rearrange IMP)
%                   Ftn = phi(:,1:end-1);
%                   Ftn_p1 = phi(:,2:end);
%                   lambda = obj.System.lambda-Lambda_K(f);% please sort
...
%         end
        ...
        W = obj.System.W;
        JOP1 = squeeze(Fn(:,:,1));
        RHS1 = JOP1-B*squeeze(WR(:,:,1,1));
        [I,F] = resonance_detection(Lambda_M_vector,Lambda_K,obj.Options.reltol); % F contains multi-index positions
        graph_m = 0;
        [Rk] = reduced_dynamics_fo(I,F,W,RHS1,l,z_k,graph_m);

        for f = Epsilon_indices(end)+1:size(K,2)
            JOP = squeeze(Fn(:,f,1));
            V = obj.System.V;
            C_k        = B*Lambda_K(f)-A;
            RHS = JOP-B*squeeze(WR(:,f,1,1));
            Rf = Rk(1:l-1,f);
            RHS = RHS - B*V(:,1:l-1)*Rf;
            W_k(:,f,1,:) = (lsqminnorm(C_k,RHS)).*ones(N,Ntime);
            R_k(:,f,1,:) = Rf.*ones(l-1,Ntime);
        end 
        W_0i = W_k;
        R_0i = R_k;
        H_k(:,:,1,:) = W_0i;
...
function [Rk] = reduced_dynamics_fo(I,F,W,Fi,l,z_k,graph)
% F - multi-index positions that are resonant
% I - Eigenvalue positions that are resonant
