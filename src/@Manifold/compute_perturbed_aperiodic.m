function [W1, R1,W0,R0] = compute_perturbed_aperiodic(obj, order,orderA,W0q,R0q,varargin)
...
if obj.Options.contribNonAuto % whether to ignore higher order
    [W1,R1] = struct_setup(obj,orderA);
else %only zeroth order nonautonomous coefficients
    [W1,R1] = struct_setup(obj,0);  
end 
...
if obj.Options.anchor
    Q_0 = sparse(l,max(size(tnew)));
    RHS = F_0;
else
    Q_0 = W_M'*F_0;
    RHS = F_0-B*V_M*Q_0;
...
%      for j = 1:numel(redConj)
        ...
        if obj.System.order == 1
             lambda = obj.System.lambda;
              ...
              V = obj.System.V;
              W = obj.System.W;
              F_ext1 = griddedInterpolant(obj.System.tforce(:),RHS.','linear');
              F_ext= @(xa) ((xa >=obj.System.tforce(1)) & (xa <=obj.System.tforce(end))).*F_ext1(xa);
              h = obj.System.dh; 
              tnew = 0:h:obj.System.tforce(end);
              data.Ntime = max(size(tnew));
              data.tnew = tnew;
%               h = obj.System.tforce(2)-obj.System.tforce(1);
...
%     end
    ...
    % ordering
    if order>0
        [W0,R0,data.H] = get_autonomous_coeffs(W0q,R0q);
    end
...
%      for i = 1:nKappa
        ...
        for k = 1:orderA-1
            ...
            [FG]  = Fext_plus_Gnl(obj,data,k,W1);
            
            % Mixed Terms
            [WR] = W1R0_plus_W0R1(data,k,W0,W1,R0,R1);
            %% Find resonant terms
            ...
            Lambda_K = sum(K .* Lambda);
%             [ev_idx, multi_idx,Lambda_K]   = resonant_terms(obj,k,kappas(:,i),Omega,'k'); % F contains multi-index pos.
            %% Set reduced dynamics
            
            if obj.System.order == 1
                [R1_ik,W1_ik] = first_order_computation(V_M,W_M,FG,WR,A,B,k,l,data,Lambda_K,obj);  
            else
            ...
            R1.R(k+1) = R1_ik(k+1);
            W1.W(k+1).coeffs = W1_ik.coeffs;
            
            if l >1
                W1.W(k+1).ind = flip(sortrows(nsumk(l,k,'nonnegative')).',2); %order k multi-indices
            else
                W1.W(k+1).ind = k;
            end
        end
...
%             W1.W(k) = coeffs_lex2revlex(W1.W(k),'TaylorCoeff');
%             R1.R(k) = coeffs_lex2revlex(R1.R(k),'TaylorCoeff');
%         end
%     end
...
function [R1_ik,W1_ik] = first_order_computation(V_M,W_M,FG,WR,A,B,k,l,data,K_lambda,obj)
% Computes the SSM coeffs and reduced dynamics coefficients using first
% order implementation
dum_check = FG-pagemtimes(full(B),WR);
R1_ik_coeff     =  pagemtimes(full(W_M)',dum_check);
% R1_ik_coeff     = sum( conj(W_M(:,I)).* ( FG(:,F) - B*(WR(:,F))));
R1_ik(k+1).coeffs    = R1_ik_coeff;
if l > 1
    R1_ik(k+1).ind    = flip(sortrows(nsumk(l,k,'nonnegative')).',2); %order k multi-indices
else
    R1_ik(k+1).ind = k;
...
RHS                 = FG-pagemtimes(full(B),WR)-pagemtimes(full(B*V_M),R1_ik_coeff);
 W1_ik.coeffs = zeros(data.N,nchoosek(k+l-1,l-1),data.Ntime);
for jt = 1:nchoosek(k+l-1,l-1)
               lambda = obj.System.lambda;
              M = obj.System.M;
              V = obj.System.V;
              W = obj.System.W;
              F_ext1 = griddedInterpolant(obj.System.tforce(:),squeeze(RHS(:,jt,:)).','linear');
              F_ext= @(xa) ((xa >=obj.System.tforce(1)) & (xa <=obj.System.tforce(end))).*F_ext1(xa);
              h = obj.System.dh; 
              tnew = 0:h:obj.System.tforce(end);
              data.Ntime = max(size(tnew));
              data.tnew = tnew;
              lambda = lambda -K_lambda(jt); % sort this properly 
%               h = obj.System.tforce(2)-obj.System.tforce(1);
%               tnew = 0:0.001:obj.System.tforce(end);
%               h = 0.001;
%               Ft  = [-obj.System.Fext;sparse(obj.System.n,max(size(obj.System.tforce)))];
...
% W1_ik.coeffs = permute(W1_ik.coeffs,[1 3 2]);
end
...
function [W1,R1] = struct_setup(obj,order)
...
W1.W(1).coeffs = zeros(N,1,max(size(tnew)));
W1.W(1).ind    = sparse(l,1);
R1.R(1).coeffs = zeros(l,1,max(size(tnew)));
R1.R(1).ind    = sparse(l,1);
% for i = 1:nKappa
...
function [F_0]            = zeroth_order_forcing(obj)
% Finding all force contributions at zeroth order

h = obj.System.tforce(2)-obj.System.tforce(1);
tnew = 0:h:obj.System.tforce(end);
N = obj.dimSystem;
F_0 = zeros(N,max(size(tnew)));
if ~isempty(obj.System.Fext.data.F_n_k(1).coeffs)
        F_0= obj.System.Fext.data.F_n_k(1).coeffs;  
end
...
function [F]                     = Fext_plus_Gnl(obj,data,k,W1)
...
Force  = zeros(data.N,z_k,Ntime);
G_nl   = zeros(data.N,z_k,Ntime);
if data.l > 1
...
%     if  n <= data.Fext_ord && ~isempty(obj.System.Fext.data.F_n_k(n).coeffs)
%         
%         F_coeff = obj.System.Fext.data.F_n_k(n).coeffs;
%         F_ind   = obj.System.Fext.data.F_n_k(n).ind.';
...
%             compute_sigma(obj.System.F(n).ind.',W1.W,k,data);
        G_nl = G_nl + pagemtimes(full(obj.System.F(n).coeffs), ...
                     compute_sigma(obj.System.F(n).ind.',W1.W,k,data));
    end
...
W1R0 = zeros(data.N,z_k,data.Ntime);
W0R1 = zeros(data.N,z_k,data.Ntime);

% Terms with order 1 SSM coefficients (in epsilon)
data.mix = 'W1';
for m = 1:k %includes the zeroth order of W1
    if  ~isempty(W1.W(m).coeffs)
        W1R0 = W1R0 + coeffs_mixed_terms_aper(k,m, W1.W, R0,data,'W1');
    end
end
...
for m = 2:k+1 % zeroth order in R1, no order k red. dyn.
    if ~isempty(R1.R(k-m+2).coeffs)
        W0R1 = W0R1 + coeffs_mixed_terms_red_aper(k,m, W0,R1.R,data,'R1');
    end
end
