function [A0, varargout] = compute_periodic_quasi_anchor(obj, order,varargin)
...
[A0,kappas,data.Fext_ord] = struct_setup(obj,order);
...
function [A0,kappas,Fext_ord] = struct_setup(obj,order)
...
A0  = repmat(struct('kappa' ,[],'A',idle,'Omega',[]),nKappa, 1);
...
for i = 1:nKappa
    ...
    A0(i).A(1).coeffs = sparse(N,1);
    A0(i).A(1).ind    = sparse(l,1);
end
end
%%
%
