function [A_0i,multi_input,timer] = cohomological_solution_periodic_perturb(obj, i,A_0,multi_input)
...
switch obj.Options.notation
    ...
    case 'multiindex'
        ...
        B   = obj.System.B; % B matrix
        N   = obj.dimSystem; % Full system dimensionality in first-order form
        l   = 1; % dim(M): M is the master modal subspace
        F_shifted   = obj.System.F_shifted; % Full system Nonlinearity coefficients at different orders
        Mi = obj.System.Minv;
...
%         end
        ...
        Ntime = max(size(multi_input.ttime));
        % The composition coefficients of power series
        H_k  = coeffs_composition_anchor(A_0,H,multi_input); % Dependent on ordering, chooses conj, or revlex computation
        H{k} = H_k;
        multi_input.H = H;
...
%                 Fn  = Fn + F(n).coeffs* ...
%                      compute_pi_anchor(F(n).ind.',K,multi_input); % Dependent on ordering, chooses conj, or revlex computation
%             end
%         end 
        %% ── Preprocessing: interpolate F_shifted coeffs onto Ntime grid ─────────
% Do this ONCE before the Fn loop, not inside it.
% t_shifted : time vector of length N_t covering [0, T]
% t_full    : time vector of length Ntime (the SSM/HB computation grid)
% Both must be available at this scope.
       %% ── Preprocessing: interpolate F_shifted coeffs onto Ntime grid ─────────
        t_shifted  = multi_input.t_periodic;          % (1, N_period) — [0, T)
        N_t_src    = length(t_shifted);
        T_period   = t_shifted(N_t_src) + (t_shifted(N_t_src) - t_shifted(max(1,N_t_src-1)));
        
        % Ntime grid — construct from multi_input
        dt_new     = T_period / Ntime;                % assumes Ntime covers one or more periods
        t_full     = (0 : Ntime-1) * dt_new;         % (1, Ntime)
        t_wrapped  = mod(t_full, T_period);           % (1, Ntime) — periodic wrap onto [0,T)
        
        for n = 2:length(F_shifted)
            if isempty(F_shifted(n)) || isempty(F_shifted(n).coeffs)
                continue
            end
            
             n_I  = F_shifted(n).n_I;
             N_t  = F_shifted(n).N_t;
             N    = size(F_shifted(n).coeffs, 1);
            
                C_mat = reshape(full(F_shifted(n).coeffs), N * n_I, N_t);
            
                if N_t == 1
                    %% ── Static cell: just repeat the single slice Ntime times ────────
                    C_new = repmat(C_mat, 1, Ntime);                   % (N*n_I, Ntime)
                else
                    %% ── Time-dependent cell: interpolate onto Ntime grid ─────────────
                    t_ext = [t_shifted(:); T_period];                  % (N_t+1, 1)
                    C_ext = [C_mat, C_mat(:, 1)];                      % (N*n_I, N_t+1)
            
                    % interp1: X=(N_t+1,1), V=(N_t+1, N*n_I), Xq=(Ntime,1)
                    C_new = interp1(t_ext, C_ext.', t_wrapped(:), 'linear');  % (Ntime, N*n_I)
                    C_new = C_new.';                                           % (N*n_I, Ntime)
                end
            
                F_shifted(n).coeffs = sparse(reshape(C_new, N, n_I * Ntime));
                F_shifted(n).N_t    = Ntime;
         end

%% ── Force accumulation loop (unchanged except N_t is now Ntime) ──────────
        Fn = sparse(N, Ntime);
        
        for n = 2:min(k, multi_input.nl_order)
            if ~isempty(F_shifted(n)) && ~isempty(F_shifted(n).coeffs) && ~isempty(F_shifted(n).ind)
        
                n_I = F_shifted(n).n_I;
                N_t = F_shifted(n).N_t;   % now equals Ntime after preprocessing
        
                Pi    = compute_pi_anchor(F_shifted(n).ind.', K, multi_input);  % (n_I, Ntime)
                Cb     = reshape(full(F_shifted(n).coeffs), N, n_I, N_t);        % (N, n_I, Ntime)
                Pi_pg = reshape(Pi, n_I, 1, N_t);                               % (n_I, 1, Ntime)
                Fn_pg = pagemtimes(Cb, Pi_pg);                                    % (N, 1, Ntime)
                Fn    = Fn + reshape(Fn_pg, N, N_t);                            % (N, Ntime)
        ...
        switch obj.Options.Solve_Method
            case 'Newmark'
                if nnz(JOP)~=0
                    Fext = -JOP(1:N/2,:);
                    F_ext = griddedInterpolant(multi_input.ttime(:),Fext.','linear');
%                     F_ext = @(xa) F_ext(xa).';
                    n = size(M,1);
                    h = obj.System.dh;
                    ...
                    residual = @(q,qd,qdd,t) residual_linear_inhomo_t( q, qd, qdd, t, M,Ks,C, F_ext,multi_input.G_ext);
                    ...
                    A_dum = zeros(2*size(TI_NL.Solution.q,1),1,1,size(TI_NL.Solution.q(1,1:end),2));
                    A_dum(:,1,1,:) = [TI_NL.Solution.q(:,1:end);TI_NL.Solution.qd(:,1:end)];
       
        
       
                    A_0i = A_dum;
                %         A_0_1(k).coeffs = A_dum;
                    H_k(:,:,1,:) = A_0i;
...
%         end
        ...
        %pass on composition coefficients
                    H{k}       = H_k;
                    timer = TI_NL.Solution.time;
                    multi_input.H = H;
                else 
                        A_0i = [];
                        timer = multi_input.ttime;
                end
        case 'Greens'
            if nnz(JOP)~=0
                    
                    t = multi_input.ttime;
                    weights = multi_input.weights;
                    V = obj.System.V;
                    W = obj.System.W;
%                     lambda = obj.System.lambda;


                    Ft  = [JOP(1:N/2,:);sparse(obj.System.n,max(size(t)))];
                    phi = W' * Ft;
%                     obj.System.Explambda = exp(lambda.*t); 
                    eta = zeros(size(phi));
                    tic
                    for j = 1:2*size(M,1)  
...
%                     A_dum(:,1,1,:) = Solution_full;
         ...
         case 'Greens_Gauss'
            if nnz(JOP)~=0
                  ...
                  WG = multi_input.weights;
                  etGM = multi_input.etime;
                  TGnew = multi_input.TTtime;
                  ...
                  A_dum = zeros(2*(obj.System.n),1,1,max(size(multi_input.ttime)));
%                     A_dum(:,1,1,:) = Solution_full;
          ...
          case 'Piecewise_exact'
            if nnz(JOP)~=0
                  ...
                  Fext = -JOP(1:N/2,:);
%                   F_ext = griddedInterpolant(multi_input.ttime(:),Fext.','spline');
                  M = obj.System.M;
                  V = obj.System.V;
                  W = obj.System.W;
                  tnew = multi_input.ttime;
                  h = multi_input.h;
                  et = multi_input.etime;
                  Ft = [-Fext;sparse(obj.System.n,max(size(Fext)))];
                  phi = W'*Ft;% (rearrange IMP)
                  Ftn = phi(:,1:end-1);
                  Ftn_p1 = phi(:,2:end);
                  ExpoA = multi_input.ExpoA;
                  ExpoB = multi_input.ExpoB;
        ...
        case 'Piecewise_exact_first_order'
            if nnz(JOP)~=0
              ...
              tnew = multi_input.ttime;
                  ...
                  A_dum = zeros(size(A,1),1,1,max(size(multi_input.ttime)));
%                     A_dum(:,1,1,:) = Solution_full;
        ...
        case 'Second_order_piecewise_exact'
            switch obj.Options.Ftype 
                case 'aperiodic' 
                    ...
                    Ft = -(multi_input.U).'*Fext;
                    eta = zeros(2*size(multi_input.U,2),max(size(tnew)));
                    np = size(multi_input.U,2);
                    ...
                    z = [multi_input.U,0*multi_input.U;0*multi_input.U,multi_input.U]*eta;
...
%                     A_dum(:,1,1,:) = Solution_full;
                ...
                case 'periodic'
                    if nnz(JOP)~=0
                        ...
                        lambda_p =multi_input.lambda_p;
%                         lambda_m =multi_input.lambda_m;
                        Fext = -JOP(1:N/2,:);
