function [res] = coeffs_mixed_terms_aperiodic(k,m,W,R,field,mix)
%%
    ...
    case 'revlex'
        ...
        if nnz(W(m).coeffs)>0 && nnz(R(u).coeffs)>0
            ...
            for j = 2:l
                ...
                Wm = W(m).coeffs(:,I_m_One,1,:);
                mRu = R(u).coeffs(j-1,I_u_One,1,:).* M(j,I_m_One);
                WR  = Wm .* mRu;
                ...
                for f = 1:z_k
                    if any(loc_k==f)
                        res(:,f,1,:) = res(:,f,1,:) + sum( WR(:,loc_k==f,1,:),2);
                    end
                end
            end
