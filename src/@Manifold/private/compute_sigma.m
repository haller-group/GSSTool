...
sigmas = sigma_vectorized_new(n_mat,K,W,field); 
...
function [sigma_out_main] = sigma_vectorized_new(N,K, W,field)
k = sum(K(:,1)); 
sigma_out = zeros(size(N,2),size(K,2));
sigma_out_main = zeros(size(N,2),size(K,2),field.Ntime);
...
for m = 0:k-1 % starting at 0, order k leads to zero pi
    ...
    if ~isempty(M)
        ...
        if nnz(cond)>0
            ...
            X_w = reshape(X_w,[],1);
            Y_w = reshape(Y_w,[],1);
            dummy_W = squeeze(W(m+1).coeffs(:,:,1));
            lin_idx_W = sub2ind(size(dummy_W),X_w,Y_w);
            
            for jt = 1:field.Ntime  
            dummy_W = squeeze(W(m+1).coeffs(:,:,jt));
            W_coeffs = dummy_W(lin_idx_W);
            W_pi = full(reshape(W_coeffs,numel(idx_K),numel(idx_N)).'.* pis);
