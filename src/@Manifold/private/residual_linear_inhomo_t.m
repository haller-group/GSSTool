function [ r, drdqdd,drdqd,drdq, c0] = residual_linear_inhomo_t( q, qd, qdd, t, M,K,C, Fext,Gext)
...
F_damping = C * qd;
F_stiff = (K+Gext(t)) * q;
r = F_inertial + F_damping +F_stiff + F_external ;
drdqdd = M;
drdqd = C;
drdq = K;
%% 
...
c0 = norm(F_inertial) + norm(F_damping) + norm(F_stiff) + norm(F_external);
end