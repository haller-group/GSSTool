function [pis] = compute_pi_anchor(n_mat,k_mat, field)
%%
...
Ntime = field.endTime;
pis = zeros(size(col_pos,2),Ntime);
...
for f = 1:z_k
    ...
    for i = 1:size(sz_un,2)
        ...
        if sz_un(i) == 0
            if nnz(K(:,f)) == 0
               pis(idx,Ntime) = ones(sumidx,Ntime);
            end
            continue
        end
        ...
        for ord = h_i_abs_un
            ...
            rmind_check = zeros(max(size(I_1)),Ntime);
            for rmind = 1:max(size(I_1))
                   part_dum =squeeze(field.H{ord}(I_1(rmind),I_2(rmind),I_3(rmind),:));
                   rmind_check(rmind,:) = part_dum;
            end
            pi_dum(I_ord,:) =  rmind_check;
%             I_1rm = repelem(I_1,Ntime);
%             I_2rm = repelem(I_2,Ntime);
%             I_3rm = repelem(I_3,Ntime);
%             lin_idx  = sub2ind([N,z_ord,ord,Ntime],I_1rm,I_2rm,I_3);
        ...
        %multiplication
        prod_pi_dum = prod(reshape(pi_dum, sz_un(i),[]),1);
        prod_pi = reshape(prod_pi_dum,size(pi_dum,1)/sz_un(i),[]).';
        ...
        idx_pos   = find(idx); 
        [xx, yy] = ndgrid(reshape(list_3,[],1),1:Ntime);
        prod_pi_t = prod_pi.';

        accum_pi  = accumarray([xx(:) yy(:)],prod_pi_t(:));        
        pis(idx_pos(n_contrib),:) = accum_pi(find(n_contrib),:);
    end
end
end
