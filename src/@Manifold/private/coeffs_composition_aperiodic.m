function [Hk] = coeffs_composition_aperiodic(A_0,H,multi_input)
...
switch multi_input.ordering
    ...
    case 'revlex'
        ...
        Hk = rev_lexicographic_computation(A_0,H,multi_input);
    case 'lex'
...
function [Hk] = rev_lexicographic_computation(A_0,H,field)
l = size(H{1},2);
k = field.k;
...
%         coeff   = A_0(ord).coeffs(:,I_gmk).* gmk_j .* H{k-ord}(:,I_g,:);
     coeff   = A_0(ord).coeffs(:,I_gmk,1,:).* gmk_j .* H{k-ord}(:,I_g,:,:);
...
%     coeff = zeros(N,sz,k-1,Ntime);
%     for time_pis = 1:Ntime
%     coeff11 = squeeze(A_0(ord).coeffs(:,:,time_pis));
%     coeff22 = squeeze(H{k-ord}(:,:,:,time_pis));
%     coeff1   = coeff11(:,I_gmk).* gmk_j .* coeff22(:,I_g,:);
%     coeff(:,:,:,time_pis) = coeff1;
%     end
%     
%     %%
%     % $\texttt{coeff} \in \mathbb{C}^{2n \times \texttt{sz} \times k-1}$. What is
...
s  = 1:k;
%match dimensions for pointiwse multiplication
Hk = permute(Hk,[2,3,1,4]) .* ((1./kj).' * s);
Hk = permute(Hk,[3,1,2,4]);

% Since we reconstruct we already know the SSM coefficients at that order
if ~isempty(A_0(k)) && ~isempty(A_0(k).coeffs)
    Hk(:,:,1,:) = A_0(k).coeffs;
end
end
