function [Mmap, iMmap, Tmap, iTmap, Nflow, yRec,fnl_rom,RMap,R,M] = fastSSM(yData, mfdorder,Vuse,use_check)
...
%% Input, settings
romorder = 11; nforder = 3; mfddim = 2;
...
for order = 1:romorder
    ...
    fnl_rom{order}.ind = exps;
    fnl_rom{order}.coeffs = R(:,Rcol:Rcol+size(exps, 1)-1);
    Rcol = Rcol + size(exps, 1);
    fnl_rom{order} = multi_index_to_tensor(fnl_rom{order}.coeffs,fnl_rom{order}.ind);  
end

% [W,Lambda] = eig(R(1:2,1:2));
% G = W\Xidot/phi(W\Xinew,1:romorder); % diagonalize R
RMap = @(t,z) R*phi(z, 1:romorder);
...
% T = [T1, T2, T3];
% Tmap = @(z) real(W*T*phi([z;conj(z)], 1:nforder));
% 
...
% iT = [iT1, iT2, iT3];
% iTmap = @(xi) iT*phi(W\xi, 1:nforder);
...
[~, zRec] = ode45(RMap, t(iStart(1):iStart(2)-1), Xi(:,1), odeset('RelTol', 1e-6));
yRec = Mmap((zRec.'));
