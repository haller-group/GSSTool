function [ r, drdqdd,drdqd,drdq, c0] = residual_nonlinear_slow( q, qd, qdd, t, M,K,C,Fnl, Fext)
...
F_damping = C * qd;
F_stiff = K * q;
r = F_inertial + F_damping +F_stiff+ F_external +F_nonlinear ;%+ F_external ;
drdqdd = M;
drdqd = C;
drdq = K;
%% 
...
c0 = norm(F_inertial) + norm(F_damping) + norm(F_stiff) +norm(F_external)+ norm(F_nonlinear) ;%+ norm(F_external);
end