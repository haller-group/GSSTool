function [M,C,K,fnl,fext, outdof,MyAssembly] = build_model_Davenport(nDiscretization)
...
% material properties
E       = 70e9;%70e9;  % 70e9 % 200e9 % Young's modulus [Pa]
rho     = 2700;%2700; % 2700 % 7850 % density [kg/m^3]
nu      = 0.33;%0.33;    % Poisson's ratio 
kappa   = 1e10; % material damping modulus 1e8
...
W =   omega(1:2);
a = [W(1) 1/W(1);W(2) 1/W(2)]\[0.04;0.04];
C = a(2) * M + a(1) * K;

MyAssembly.DATA.C = C;
%% external force assembly
disp('Assembling external force vector')
outcoord = [l/2,b/2]; % output coordinate
