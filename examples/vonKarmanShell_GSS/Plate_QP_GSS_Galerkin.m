%% Von Karman shell — quasi-periodic forcing: aSSM anchor vs. Galerkin ROM vs. full-order
...
% against both a full-order (Newmark) time integration and a
% Galerkin-projected reduced-order model built on the same truncated
% modal basis used for the GSS reduction.
%
% For the forcing-weakness ratio r_w, see the companion script
% rw_calculation_QP.m. For the non-Galerkin comparison, see
% Plate_QP_GSS.m.
...
hold on
plot(TI_GAL.Solution.time,GAL_q_full(indexRd,:),'-','LineWidth',3,'color',[0.4 0 0.4 0.5])
...
%% Plot: response torus in 3D (full-order vs. Galerkin ROM vs. O(1)/O(4) aSSM anchor)
zz_axis = 662;
figure
