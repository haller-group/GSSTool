...
run ../../install.m
%% rw_calculation_chaotic.m
%
% Computes the forcing-weakness ratio r_w for the chaotically forced von
% Karman plate example (companion to Plate_chaotic_GSS_Galerkin.m),
...
%% Build chaotic forcing (must match Plate_chaotic_GSS_Galerkin.m exactly)
L = 0;
N_Max = 1000;
tspan = linspace(L,100,N_Max);
loren = @(t,y) rossler(t,y);
IC_lorenz = [0;0.3;0.5];
[tk,yy] = ode45(loren,tspan,IC_lorenz);
...
epsilon = 12000; % must match the epsilon used in Plate_chaotic_GSS_Galerkin.m
...
disp(' ')
disp(['r_w (chaotic forcing case, averaged over ' num2str(n_IC) ' random ICs) = ' num2str(rw)])
